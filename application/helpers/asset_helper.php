...
function set_css_asset_head($css_asset, $module_name = NULL){
  ...
  if(!is_array($css_asset)){
    if($module_name == NULL)
      $obj->get_ci()->template->css_assets_head[]=js_asset($css_asset);
    else
      $obj->get_ci()->template->css_assets_head[]=js_asset($css_asset,$module_name);
  }else{
    foreach($css_asset as $key=>$data){  
      if($module_name == NULL)
        $obj->get_ci()->template->css_assets_head[]=js_asset($data);
      else
        $obj->get_ci()->template->css_assets_head[]=js_asset($data,$module_name);
    }
  }
}
