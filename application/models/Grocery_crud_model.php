...
class grocery_CRUD_Model  extends CI_Model  {
    ...
    function get_list()
    {
        ...
        /*
         *  SELECT FROM JOIN STATEMENT START
         */
		// ADD SELECT FROM JOIN HERE <-----------------------------------------
		// example: $select .= ", `user_log`.`created_date`, `user_log`.`update_date`";
		if ($this->table_name === 'product') {
            // if ($this->db->query(" SELECT * FROM product_stock ")->num_rows() > 0)
    			// $select .= ", `product_stock`.*";
                // $select .= ", `product_stock_tmp`.*";
        }
        else if ($this->table_name === 'order') {
            // if ($this->db->query(" SELECT * FROM order_list ")->num_rows() > 0 &&
            //     $this->db->query(" SELECT * FROM order_address ")->num_rows() > 0)
            //     $select .= ", `order_list`.*, `order_address`.*";
            if ($this->db->query(" SELECT * FROM order_address ")->num_rows() > 0)
                $select .= ", order_address.OD_Name";
        }
        /*
         *  SELECT FROM JOIN STATEMENT END
         */
        ...
        /*
         *  JOIN STATEMENT START
         */
		// ADD JOIN HERE <----------------------------------------------------
		// example: $this->db->join('user_log','user_log.user_id = users.id');
		if ($this->table_name === 'product') {
            // if ($this->db->query(" SELECT * FROM product_stock ")->num_rows() > 0)
                // JOIN NORMAL CONDITION
                // $this->db->join('`product_stock`', '`product_stock`.`P_ID` = `product`.`P_ID`', 'left');
                
                // JOIN DISTINCT AND ORDER BY LASTES DATETIMEUPDATE CONDITION
                // $this->db->join(
                //     '(SELECT `PS1`.* FROM `product_stock` as `PS1` LEFT JOIN `product_stock` AS `PS2` ON `PS1`.`P_ID` = `PS2`.`P_ID` AND `PS1`.`PS_DateTimeUpdate` < `PS2`.`PS_DateTimeUpdate` WHERE `PS2`.`P_ID` IS NULL) as `product_stock_tmp`', 
                //     '(`product`.`P_ID` = `product_stock_tmp`.`P_ID`)', 
                //     'left'
                // );
        }
        else if ($this->table_name === 'order') {
            // if ($this->db->query(" SELECT * FROM order_list ")->num_rows() > 0 &&
            //     $this->db->query(" SELECT * FROM order_address ")->num_rows() > 0) {
            //     $this->db->join('`order_list`',     '`order_list`.`OD_ID` = `order`.`OD_ID`', 'left');
            //     $this->db->join('`order_address`',  '`order_address`.`OD_ID` = `order`.`OD_ID`', 'left');
            // }
            if ($this->db->query(" SELECT * FROM order_address ")->num_rows() > 0)
                $this->db->join('order_address',  'order_address.OD_ID = order.OD_ID');
        }
        /*
         *  JOIN STATEMENT END
         */
